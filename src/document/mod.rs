...
fn table_row(columns: usize, first: bool, settings: std::sync::Arc<table::TableSettings>) -> Block {
    Block::TableRow {
...
impl Document {
    ...
    pub fn table_tab(&mut self, backwards: bool) -> bool {
        ...
        let columns = self.scope()[block].cells().len();
        let row = block - first;
    ...
    pub fn toggle_table_line(&mut self, block: usize, line: table::GridLine) -> bool {
        ...
        let columns = self.scope()[first].cells().len();
        let rows = end - first;
